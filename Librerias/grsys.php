...
class GR
   ...
   var $cant_recibo;          // Cantidad de recibos de una OT
	...
	function GuardarPresOT()
		...
		// Bucle que guarda los items MAT
	  $suma_mat=0;
	  $this->con_sql->Execute("DELETE FROM otd WHERE nro_ot=$this->nro AND item<>0");
...
<div id="parentItems" style="width: 90%; margin: 10px auto;">
	...
	function ArmarMOB($tipo, $nro, $edit)
		...
		if(!$edit)
			...
			while(!$res->EOF)
				...
				$this->item_mob[($i++)] = new ITEM(0, $res->fields['numero'], $res->fields['cantidad'], 0, htmlentities($res->fields['descripcion']), $res->fields['importe'], $agregado, $res->fields['fecha_real']);
				$res->MoveNext();
				}
   ...
   <?php
	...
	function ItemSetCantidad($item, $numero, $agregado, $cantidad)
		...
		else			$this->item_mat[$i]->SetCantidad($cantidad);
		?>
		<script language="javascript">
...
class ITEM
	...
	//****************************************** FUNCIONES ******************************************
	// CONSTRUCTOR
	function ITEM($rubro, $numero, $cantidad, $codigo, $descripcion, $importe, $agregado,$fecha_real)
		{
	...
	function Mostrar($tipo, $edit)
        ...
        if ($this->rubro==0) {
            ?>
        <td><?php echo($this->realizado<>'0000-00-00' ? date_format(date_create($this->realizado), 'd/m/Y') : ''); ?></td>
        <?php
        }
	}
